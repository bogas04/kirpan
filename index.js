...
function onGeoLocation() {
  const { getLocation, API_URL, transformGeoParams, transformResponseToCountry } = util.geo;

  const previousValue = $geoLocationButton.innerText;
  $geoLocationButton.innerText = 'Detecting...';

  getLocation()
    .then(({ coords: { latitude, longitude } }) => fetch(`${API_URL}?${transformGeoParams(latitude, longitude)}`))
    .then(r => r.json())
...
function onQueryInput(query = '') {
  ...
  if (data.length > 0) {
    data.forEach(([country, info]) => $results.appendChild(
      h('div', {}, [
        h('h2', {}, [
          h(
            'small',
            { title: info.maybe === true ? 'Maybe' : (info.allowed ? 'Allowed' : 'Not allowed'), },
            info.maybe === true ? '?' : (info.allowed ? '✓' : '✗')
          ),
          info.formalName,
        ]),
        h('ul', { style: 'list-style: none' }, info.laws
          .map(([law, source]) => h('li', {}, [
...
['change', 'input'].forEach(event => $query.addEventListener(event, onQueryInput));
